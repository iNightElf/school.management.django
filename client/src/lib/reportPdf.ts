...
export async function downloadReportCardPDF(student: any, clsName: string, subjects: any[], allResults: any[], term: string, sharedDoc?: any) {
  ...
  if (student.hasPhoto) {
    try { const r = await api.get(`/students/${student.id}/photo/`, { responseType: 'blob', headers: { 'X-Proxy': '1' } }); const blob = r.data; photoDataUri = await new Promise<string>(res => { const reader = new FileReader(); reader.onload = () => res(reader.result as string); reader.readAsDataURL(blob); }); } catch { console.warn('Photo fetch failed for', student.id); }
  }
